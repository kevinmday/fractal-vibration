...
from agents import IntentionAgent, ChaosAgent

intention_agent = IntentionAgent()
chaos_agent = ChaosAgent()

while True:
    data = get_market_data()  # or any event/input
    intention = intention_agent.analyze(data)
    chaos = chaos_agent.analyze(data)
    
    # Integrate both agents into a single decision metric
    effective_force = intention["score"] * (1 - chaos["chaos_level"])
    print(f"Effective Force: {effective_force:.3f}")
    
    make_decision(effective_force)  # plug into your action layer
