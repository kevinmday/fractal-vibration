...
# Assume F_int_sent is computed externally and passed in or accessible globally
def calculate_metrics(data, f_int_sent=None):
    ...
    # Integrate intention-sentiment force as an influence on intention
    if f_int_sent is not None:
        # Weighted adjustment of intention score (subject to tuning)
        adjusted_intention = intention_score * (1 + (f_int_sent / 100))
        adjusted_intention = round(adjusted_intention, 2)
    else:
        adjusted_intention = intention_score

    return {
        "price": round(latest_price, 2),
        "intention": adjusted_intention,
        "scale": scale_score,
        "ttcf": ttcf_score,
