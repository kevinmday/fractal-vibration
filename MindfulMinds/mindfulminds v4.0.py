...
USE_MOCK_DATA = False  # Toggle for mock or live API

# NEW: Live ticker function
...
def price():
    ...
    return jsonify({"Bitcoin Price (USD)": btc_price})
...
# Calculate metrics (updated for intuitive 0–100 scaling)
def calculate_metrics(data):
    ...
    # Intention: movement from oldest to latest
    raw_intention = (latest_price - oldest_price) / oldest_price
    intention_score = round(abs(raw_intention) * 100, 2)  # Normalize to 0–100

    # Scale: short-term movement from previous close
    raw_scale = (latest_price - prev_price) / prev_price
    scale_score = round(abs(raw_scale) * 100, 2)  # Normalize to 0–100

    # Chaos Factor (TTCF): inverse of intention, scaled
    raw_ttcf = (1 / (abs(raw_intention) + 0.001)) * 1.5
    ttcf_score = min(100, round(raw_ttcf, 2))  # Cap at 100

    return {
        "price": round(latest_price, 2),
        "intention": intention_score,  # 0–100
        "scale": scale_score,          # 0–100
        "ttcf": ttcf_score              # 0–100
    }
