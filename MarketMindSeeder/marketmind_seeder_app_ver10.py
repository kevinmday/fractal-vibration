...
# Mount static directory only once
...
# ---------- BACKGROUND SEEDING ----------
def auto_seed_from_web():
    pyramid_targets = {
        "MARKET": ["SPY", "QQQ", "VIX", "DXY"],
    ...
    for level, symbols in pyramid_targets.items():
        for symbol in symbols:
            ...
            notes = f"{level} update seeded via bot"
            now = datetime.utcnow().isoformat()
            c.execute("""
                INSERT INTO asset_intention (symbol, timestamp, direction, scale, notes, level, source)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (symbol, now, direction, scale, notes, level, "autobot"))
    conn.commit()
    print("✅ Pyramid seeding cycle completed.")

scheduler = BackgroundScheduler()
scheduler.add_job(auto_seed_from_web, 'interval', minutes=10)
scheduler.start()
...
def mock_update():
    ...
    for symbol in symbols:
        ...
        c.execute("""
            INSERT INTO asset_intention (symbol, timestamp, direction, scale, notes, level, source)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (symbol, now, direction, scale, notes, "ASSET", "mock"))
    conn.commit()
...
if __name__ == "__main__":
    ...
    if args.run or (not args.seed and not args.run):
        uvicorn.run("marketmind_seeder_app_ver9:app", host="127.0.0.1", port=8000, reload=True)
