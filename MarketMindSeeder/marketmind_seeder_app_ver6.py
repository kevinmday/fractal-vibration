...
def mock_update():
    ...
    for symbol in symbols:
        ...
        now = datetime.now(timezone.utc).isoformat()  # Assign here
        c.execute("""
            INSERT INTO asset_intention (symbol, timestamp, direction, scale, notes)
            VALUES (?, ?, ?, ?, ?)
        """, (symbol, now, direction, scale, notes))
...
if __name__ == "__main__":
    # Manually control flags from within the script
    RUN_SERVER = True
    SEED_DATA = True
    ...
    if RUN_SERVER:
        uvicorn.run("marketmind_seeder_app_ver6:app", host="127.0.0.1", port=8000, reload=True)
